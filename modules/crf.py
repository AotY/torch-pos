...
"""
fork from https://github.com/s14t284/TorchCRF/blob/master/TorchCRF/__init__.py
"""
...
class CRF(nn.Module):
    def __init__(
        self, tgt_size: int,
        pad_idx: Optional[int] = None) -> None:
        """
        :param tgt_size: number of labels
        ...
        if tgt_size < 1:
            raise ValueError("invalid number of labels: {0}".format(tgt_size))
        ...
        self.trans_matrix = FloatTensor(tgt_size, tgt_size)
        # 先頭と末尾への遷移行列の設定
        # transition matrix of start and end settings
        self.start_trans = FloatTensor(tgt_size)
        self.end_trans = FloatTensor(tgt_size)
    ...
    def forward(self,
        ...
        """
        :param h: hidden matrix (batch_size, seq_len, tgt_size)
    ...
    def _viterbi_compute_best_path(
        ...
        :param score: transition scores of length max sequence size
                      in mini batch [(batch_size, tgt_size)]
    ...
    def _compute_denominator_log_likelihood(self, h: FloatTensor, mask: BoolTensor):
        ...
        batch_size, seq_len, _ = h.size()
        # 計算できるよう，遷移行列のサイズを変更
        # (tgt_size, tgt_size) -> (1, tgt_size, tgt_size)
        trans = self.trans_matrix.view(1, self.tgt_size, self.tgt_size)
        ...
        for t in range(1, seq_len):
            # (batch_size, self.tgt_size, 1)
            before_score = score.view(batch_size, self.tgt_size, 1)
            # 各系列の系列のt番目のマスクを用意
            ...
            # in each sequence
            # (batch_size, 1, tgt_size)
            h_t = h[:, t].view(batch_size, 1, self.tgt_size)
            # 各系列でのt番目のスコアを導出
    ...
    def _compute_numerator_log_likelihood(
        ...
        h = h.unsqueeze(-1).view(batch_size, seq_len, self.tgt_size)
